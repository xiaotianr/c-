...
debug版本上（调试时），inline是不起作用的； inline只有在release版本下才能出现 
...
int main()
{
	...
	const int *p3 = new const int(40);//堆上生成常量对象的new

	// 定位new  
	int data = 0;
	int *p4 = new (&data) int(50);//在指定的内存上，划分一个4字节大小的内存，初值设为50
